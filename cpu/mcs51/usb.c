...
uint8 XDATA usbDeviceState = 0;

uint8 XDATA controlTransferState = 0;
        ...
        if (usbcs0 & (1<<0))  // Check OUTPKT_RDY
        {
            ...
            if (controlTransferState == 1)
            {
                ...
                else
                {
                    ...
                    if (usbSetupPacket.requestType != 0u) // OPT: remove this check
                    {
            ...
            else if (USBCNT0 == 8)
            {
                ...
                usbReadFifo(0, 8, (uint8 *)&usbSetupPacket); // Store the data in usbSetupPacket.

                // Wipe out the information about the last control transfer.
                controlTransferState = 0;

                if (usbSetupPacket.requestType == 0u)
                {
...
// Assumption: controlTransferState is 0 when this function is called.
//static void usbStandardDeviceRequestHandler()
void usbStandardDeviceRequestHandler()
{
    // Prepare a convenient two-byte buffer for sending 1 or 2 byte responses.
    //static uint8 response[2];
    uint8 XDATA response[2];
    response[0] = 0;
    response[1] = 0;
    ...
    {
        case 6u: // USB Spec 9.4.3 Get Descriptor
        {
        ...
        case 5u: // USB Spec, 9.4.6 Set Address
        {
            ...
            usbDeviceState = ((uint8)usbSetupPacket.wValue) ? 8 : 4;

            // Get ready to provide a handshake.
            usbControlAcknowledge();
            return;
        }
        case 9u: // USB Spec, 9.4.7 Set Configuration
        {
        ...
        case 8u: // USB Spec 9.4.2 Get Configuration
        {
            // Assumption: there is only one configuration and its value is 1.
            response[0] = (usbDeviceState == 16) ? 1 : 0;
            usbControlRead(1, response);
            return;
        }
        case 10u: // USB Spec 9.4.4 Get Interface
        {
        ...
        case 0u: // USB Spec 9.4.5 Get Status
        {
            switch(usbSetupPacket.recipient)
            {
                ...
                case 1u:
                {
                    if (usbDeviceState < 16 && usbSetupPacket.wIndex != 0)
                    {
...
// usbDeviceState == 2 and it will draw more power than it should from USB.
