...
int sched_run(void)
{
    ...
    active_thread->sp = sched_active_thread->sp;
    active_thread->status = sched_active_thread->status;
    active_thread->priority = sched_active_thread->priority;
    active_thread->pid = sched_active_thread->pid;
    #ifdef MODULE_CORE_THREAD_FLAGS
    active_thread->flags = sched_active_thread->flags;
    #endif
    active_thread->rq_entry.next = sched_active_thread->rq_entry.next;
    #if defined(MODULE_CORE_MSG) || defined(MODULE_CORE_THREAD_FLAGS) \
    || defined(MODULE_CORE_MBOX)
    active_thread->wait_data = sched_active_thread->wait_data;
    #endif
    #if defined(MODULE_CORE_MSG)
    active_thread->msg_waiters = sched_active_thread->msg_waiters;
    active_thread->msg_queue = sched_active_thread->msg_queue;
    active_thread->msg_array = sched_active_thread->msg_array;
    #endif
    #if defined DEVELHELP || defined(SCHED_TEST_STACK)
    active_thread->stack_start = sched_active_thread->stack_start;
    #endif
    #ifdef DEVELHELP
    active_thread->name = sched_active_thread->name;
    active_thread->stack_size = sched_active_thread->stack_size;
    #endif
